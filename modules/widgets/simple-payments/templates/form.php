<p>
	<label for="<?php echo esc_attr( $this->get_field_id( 'widget_title' ) ); ?>"><?php echo esc_html_e( 'Widget Title' ); ?></label>
	<input
		type="text"
		class="widefat simple-payments-widget-title"
...
<p>
	<label for="<?php echo $this->get_field_id('product_post_id'); ?>"><?php _e( 'Select a Simple Payment Button:' ); ?></label>
	<select class="widefat simple-payments-products" id="<?php echo $this->get_field_id('product_post_id'); ?>" name="<?php echo $this->get_field_name('product_post_id'); ?>">
	<?php foreach ( $product_posts as $product_post ) { ?>
		<option value="<?php echo esc_attr( $product_post->ID ) ?>"<?php selected( (int) $instance['product_post_id'], $product_post->ID ); ?>><?php echo esc_attr( get_the_title( $product_post ) ) ?></option>
	<?php } ?>
	</select>
</p>
<?php } ?>
<p>
	<div class="alignleft">
		<button class="button simple-payments-edit-product"><?php esc_html_e( 'Edit' ); ?></button>
	</div>
	<div class="alignright">
		<button class="button simple-payments-add-product"><?php esc_html_e( 'Add New' ); ?></button>
	</div>
	<br class="clear">
</p>
<hr />
<div class="simple-payments-form" style="display: none;">
	<input
		type="hidden"
		id="<?php echo $this->get_field_id('form_action'); ?>"
		name="<?php echo $this->get_field_name('form_action'); ?>"
		class="simple-payments-form-action" />
	<input
		type="hidden"
